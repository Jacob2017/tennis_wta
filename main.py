...
def run_date(date, matches_df, rankings_df, level=1):
    ...
    for player in matches_df["winner_name"].unique():
        player_df = matches_df[
            (matches_df["winner_name"] == player) | (matches_df["loser_name"] == player)
        ]
        player_rank_df = rankings_df[
            (rankings_df["player_name"] == player)
            & (rankings_df["tourney_date"] >= date - 10000)
        ]
        if len(player_rank_df) != 0:
            max_rank = player_rank_df[
                player_rank_df["tourney_date"] == max(player_rank_df["tourney_date"])
            ]
        else:
            max_rank = None
        ...
        k_factor = 800 / (ne + m + 8)
        total_score = (
            player_df[player_df["winner_name"] == player]["s1"].sum()
            + player_df[player_df["loser_name"]["s2"]].sum()
        )
        avg_score = total_score / m
        dp = DP_LOOKUP[round(avg_score, 2)]
        opponents = list(
            matches_df[matches_df["winner_name"] == player]["loser_name"]
        ) + list(matches_df[matches_df["loser_name"] == player]["winner_name"])
        if len(rankings_df) > 0:
            opponents_rankings = list(
                rankings_df[
                    (rankings_df["player_name"].isin(opponents))
                    & (rankings_df["tourney_date"] == max(rankings_df["tourney_date"]))
                ]["rating_end"]
            )
        else:
            opponents_rankings = []
